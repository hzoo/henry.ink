...
export const useAtCute = () => {
	useEffect(() => {
		...
		const processLoginOrLoadSession = async () => {
			...
			if (
				!isBrowserExtension &&
				(location.hash.includes("state=") || location.search.includes("code="))
			) {
				try {
			...
			if (persistedDid) {
				try {
					console.log(
						`Attempting to load session for persisted DID: ${persistedDid}`,
					);
					...
					if (session && isMounted) {
						console.log(
							"Successfully loaded/refreshed session:",
							session.info.sub,
						); // Log only sub for brevity
						const agent = new OAuthUserAgent(session);
						const rpc = new Client({ handler: agent });
						atCuteState.value = { agent, rpc, session };
					} else if (!session) {
						console.warn(
							"getSession returned null (likely refresh failure), clearing potentially invalid session for DID:",
							persistedDid,
						);
						localStorage.removeItem("atcute-oauth:did");
						try {
							...
							console.log(
								"Called deleteStoredSession for potentially invalid DID:",
								persistedDid,
							);
						} catch (deleteErr) {
					...
					if (
						...
						console.error(
							`Error during getSession for DID ${persistedDid}:`,
							error,
						);
					} else {
						console.log(
							`No valid session found via getSession for DID ${persistedDid}.`,
						);
					}
					localStorage.removeItem("atcute-oauth:did");
					try {
						...
						deleteStoredSession(persistedDid as `did:${string}:${string}`);
						console.log(
							"Called deleteStoredSession after getSession check for DID:",
							persistedDid,
						);
					} catch (deleteErr) {
...
export const startLoginProcess = async (handleOrDid?: string) => {
	try {
		...
		if (isBrowserExtension) {
			...
			console.log(
				"launchWebAuthFlow successful, final callback URL:",
				finalRedirectUrl,
			);
		...
		if (!(errorMsg.includes("cancelled") || errorMsg.includes("user closed"))) {
			alert(
				`Failed to complete login process: ${error instanceof Error ? error.message : String(error)}`,
			);
		} else {
...
export const logout = async () => {
	...
	if (currentAgent) {
		try {
			...
			if (did) {
				try {
					...
					console.log(
						"Fallback deleteStoredSession successful after signOut error.",
					);
				} catch (deleteError) {
		...
		try {
			...
			console.error(
				"deleteStoredSession failed (no agent available):",
				deleteError,
			);
		}
	}
