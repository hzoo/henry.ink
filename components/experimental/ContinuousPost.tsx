	...
	const renderKeyboardHelp = () => (
		<div className="p-3 bg-gray-50 dark:bg-gray-800 text-xs text-gray-600 dark:text-gray-300 rounded-md mb-4">
			...
			<div className="grid grid-cols-2 gap-x-4 gap-y-1">
				...
				<div>
					<kbd>b</kbd> Toggle breadcrumbs
				</div>
				<div>
					<kbd>s</kbd> Toggle siblings
				</div>
			</div>
		</div>
	);
		...
		<div className="flex flex-col">
			...
			>
							...
							<div key={segment.path} className="flex items-center min-w-0">
								...
								>
									{displayItems.includes("avatar") &&
										segment.node.post.author.avatar && (
											<img
									...
									<span className="truncate max-w-[50px] text-xs">
										{segment.node.post.author.displayName ||
											segment.node.post.author.handle}
									</span>
								</button>
							</div>
						))}
				...
				<div className="divide-y divide-gray-100 dark:divide-gray-700">
					{/* Active post */}
					{activeNode.value &&
						!applyFilters(activeNode.value.post, filters) && (
							<div className="px-4 py-3">
								{/* Author info and metadata */}
								<div className="flex justify-between items-start mb-2">
									<div className="flex items-center gap-2">
										{displayItems.includes("avatar") &&
											activeNode.value.post.author.avatar && (
												<img
													src={activeNode.value.post.author.avatar}
													alt={
														activeNode.value.post.author.displayName ||
								...
								<div className="flex flex-wrap gap-2 text-xs">
									...
									{activeRelations.value.parent && (
										<button
											onClick={() =>
												(activeCursor.value =
													activeRelations.value.parent!.path)
											}
									...
									{activeRelations.value.siblings.length > 0 && (
										...
										>
											...
											<span>
												Siblings ({activeRelations.value.siblings.length})
											</span>
										</button>
									)}
									...
									{activeRelations.value.children.length > 0 && (
										<button
											onClick={() =>
												(activeCursor.value =
													activeRelations.value.children[0].path)
											}
											className="flex items-center gap-1 px-2 py-1 bg-gray-100 dark:bg-gray-700 rounded-md hover:bg-gray-200 dark:hover:bg-gray-600"
										>
											<Icon name="comment" className="size-3" />
											<span>
												Replies ({activeRelations.value.children.length})
											</span>
										</button>
									)}
