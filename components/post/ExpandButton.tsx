...
export function ExpandButton({ post, isExpanded }: ExpandButtonProps) {
	...
	return (
		<div
			className={isExpanded.value ? "absolute left-1 top-6.5 bottom-1 w-max flex flex-col items-center cursor-pointer z-10" : "cursor-pointer"
			}
		...
		>
			{isExpanded.value ? (<div className={`w-[1.5px] h-1 transition-colors duration-150 ${
			...
			/>) : null}
				...
				{isExpanded.value ? (
					<Icon name="minusCircle" className={isExpanded.value ? "size-4" : "size-6"} />
				) : (
					<Icon name="plusCircle" className={isExpanded.value ? "size-4" : "size-6"} />
				)}
			</span>
			<div
