...
import defaultMetadata from './public/oauth/client-metadata.json' with { type: 'json' };
import henryInkMetadata from './note-site/public/oauth/client-metadata.json' with { type: 'json' };
...
function getOAuthConfig(target: BuildTarget): OAuthConfig {
  ...
  if (!config) {
    throw new Error(`Unknown build target: ${target}. Available targets: ${Object.keys(TARGET_CONFIGS).join(', ')}`);
  }
  return config;
}
...
function validateAndGetUris(metadata: OAuthConfig) {
  ...
  if (!WEB_PROD_REDIRECT_URI) {
    throw new Error(`Could not find the main web redirect URI (matching client_uri) in OAuth config for ${metadata.client_uri}`);
  }
  if (!EXT_CALLBACK_REDIRECT_URI) {
    throw new Error(`Could not find the extension callback redirect URI (.../oauth/callback) in OAuth config for ${metadata.client_uri}`);
  }
  if (!FF_EXT_CALLBACK_REDIRECT_URI) {
    throw new Error(`Could not find the Firefox extension callback redirect URI (.../ff/oauth/callback) in OAuth config for ${metadata.client_uri}`);
  }
...
export function injectOauthEnvForExtension(browser: string): PluginOption {
  const target: BuildTarget = browser === 'firefox' ? 'extension-firefox' : 'extension-chrome';
  return injectOauthEnv(target);
}

export function injectOauthEnv(target: BuildTarget = 'annotation-demo'): PluginOption {
  return {
    name: 'inject-oauth-env',
    config: (_config: UserConfig, { command }: { command: string }) => {
      const metadata = getOAuthConfig(target);
      const { WEB_PROD_REDIRECT_URI, EXT_CALLBACK_REDIRECT_URI, FF_EXT_CALLBACK_REDIRECT_URI } = validateAndGetUris(metadata);
