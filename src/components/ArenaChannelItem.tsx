...
import type { ArenaMatch, ArenaBlock, ImageBlock, TextBlock, LinkBlock, EmbedBlock, AttachmentBlock } from "@/src/lib/arena-types";
...
function ArenaBlockItem({ block }: { block: ArenaBlock }) {
  ...
  switch (block.__typename) {
    case 'Image':
      ...
      const imageUrl = imageBlock.resized_image?.grid_cell_resized_image?.src_1x;
        ...
        >
          <img 
            ...
            onError={(e) => {
              ...
              if (parent) {
                parent.innerHTML = '<div class="flex items-center justify-center w-full h-full text-gray-500 dark:text-gray-400 text-xs">IMG</div>';
              }
            }}
          />
        </div>
    ...
    case 'Text':
      ...
      const content = textBlock.content?.replace(/<[^>]*>/g, '').trim() || textBlock.title || 'Text block';
    ...
    case 'Link':
      ...
      const linkImageUrl = linkBlock.resized_image?.grid_cell_resized_image?.src_1x;
      const domain = linkBlock.source?.url ? new URL(linkBlock.source.url).hostname : 'Link';
    ...
    case 'Embed':
      ...
      const embedImageUrl = embedBlock.resized_image?.grid_cell_resized_image?.src_1x;
    ...
    case 'Attachment':
      ...
      const attachmentImageUrl = attachmentBlock.resized_image?.grid_cell_resized_image?.src_1x;
...
export function ArenaChannelItem({ match, index, matchTextCounts, viewMode }: ArenaChannelItemProps) {
  ...
  return (
    <div className="p-3 border-b border-gray-100 dark:border-gray-800 hover:bg-gray-50 dark:hover:bg-gray-800/50 transition-colors group">
      ...
      <div 
        ...
        title={`Match: "${match.matchedText}"${match.context ? `\nContext: ...${match.context}...` : ''}`}
      ...
      >
        <a 
          ...
          className="text-green-700 dark:text-green-400 hover:text-green-800 dark:hover:text-green-300 font-medium text-xs leading-tight truncate mr-2 hover:underline"
        ...
        <div className="flex items-center gap-2 flex-shrink-0">
          {totalMatches > 1 && (
            <span className="bg-gray-100 dark:bg-gray-700 text-gray-600 dark:text-gray-300 text-xs px-1.5 py-0.5 rounded-full font-medium">
              {totalMatches}
            </span>
          )}
          <span className="bg-yellow-100 dark:bg-yellow-900/30 text-yellow-800 dark:text-yellow-200 text-xs px-1.5 py-0.5 rounded font-medium group-hover:bg-yellow-200 dark:group-hover:bg-yellow-900/50 transition-colors">
            "{match.matchedText.length > 20 ? match.matchedText.substring(0, 20) + '...' : match.matchedText}"
          </span>
        </div>
      </div>
      
      {/* Blocks Grid - Only in Preview Mode */}
      {viewMode === 'preview' && (
        <div>
          {blocksLoading ? (
            ...
            <div className="grid grid-cols-2 gap-1.5">
              {blocks.slice(0, 4).map((block) => (
                <ArenaBlockItem key={block.id} block={block} />
              ))}
            </div>
          ) : (
            <div className="text-xs text-gray-500 dark:text-gray-400 italic py-2">
