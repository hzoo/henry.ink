...
export function CardStack({ navigator, displayItems }: CardStackProps) {
	...
	useEffect(() => {
		...
		const getPathToCursor = (uri: string): string[] => {
			...
			while (current) {
				...
				current = current.parentUri
					? navigator.getNode(current.parentUri)
					: null;
			}
		...
		const isCursorVisible =
			currentTopNode?.uri === currentCursor ||
		...
		>
			...
			{stack.value.map((node, idx) => {
				...
				return (
					...
					>
						<div
							className={`border border-gray-200 dark:border-gray-700 rounded-md shadow-md h-full overflow-y-auto ${
								isCurrentCard
						...
						>
							<div
								className={`p-2 flex items-center justify-between sticky top-0 border-b border-gray-200 dark:border-gray-700 z-10 ${
									isCurrentCard
