...
export function FileView({ navigator, displayItems, filters }: FileViewProps) {
	...
	const threadPath = useComputed(() => {
		...
		return pathUris
			.map((uri) => navigator.getPost(uri))
			.filter(Boolean) as AppBskyFeedDefs.PostView[];
	});
	...
	const adjacentLeaves = useComputed(() => {
		...
		if (currentIndex === -1) {
			...
			for (
				let i = chronoIndex + 1;
				i < navigator.chronologicalUris.length;
				i++
			) {
		...
		return {
			...
			next:
				currentIndex < leafNodes.value.length - 1
					? leafNodes.value[currentIndex + 1]
					: null,
		};
	});
	...
	const relatedNodes = useComputed(() => {
		...
		const siblingUris =
			parent?.childUris.filter((uri) => uri !== node.uri) || [];
		const siblings = siblingUris
			.map((uri) => navigator.getPost(uri))
			.filter(Boolean) as AppBskyFeedDefs.PostView[];

		// Get children
		const childPosts = node.childUris
			.map((uri) => navigator.getPost(uri))
			.filter(Boolean) as AppBskyFeedDefs.PostView[];
		...
		const handleKeyDown = (e: KeyboardEvent) => {
			...
			switch (e.key) {
				...
				case "j": {
					...
					const nextLeaf = findNextLeafInTraversal(
						navigator,
				...
				case "k": {
					...
					const prevLeaf = findPrevLeafInTraversal(
						navigator,
						navigator.cursor.value,
					);
					if (prevLeaf) {
	...
	const renderTreeNode = (uri: string, level: number): React.ReactNode => {
		...
		return (
			<div key={uri} className="mb-1">
				...
				>
					{isLeaf ? (
						<span className="w-3 mr-1 text-green-600 dark:text-green-400">
							•
						</span>
					) : (
					...
					<div className="bg-gray-50 dark:bg-gray-750 px-3 py-2 border-b border-gray-200 dark:border-gray-700 flex items-center justify-between">
						<div className="flex items-center gap-2">
							...
							<div className="text-xs text-gray-500">
								{navigator.getCurrentPosition().index + 1} /{" "}
								{navigator.getCurrentPosition().total}
							</div>
						</div>
													...
													{relatedNodes.value.siblings
														...
														.map((sibling) => (
															<button
																...
																className="flex items-center gap-1 px-2 py-1 bg-gray-100 dark:bg-gray-700 rounded hover:bg-gray-200 dark:hover:bg-gray-600"
																title={
																	sibling.author.displayName ||
																	sibling.author.handle
																}
															>
																{displayItems.includes("avatar") &&
																sibling.author.avatar ? (
																	<img
