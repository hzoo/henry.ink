...
import { domainSettings, setDomainStatus } from "@/src/lib/settings";
import type { DomainStatus } from "@/src/lib/settings";

interface Props {
	onClose: () => void;
}

const handleSetDomainStatus = async (
	domain: string,
...
export function DomainManager({ onClose }: Props) {
	...
	const modal = (
		...
		>
			<div className="bg-white dark:bg-gray-800 rounded-lg w-full max-w-md mx-4 flex flex-col max-h-[min(32rem,90vh)]">
				{/* Fixed Header */}
							...
							{currentSettingsArray.map(([domain, status]) => (
								...
								>
									<span className="text-sm text-gray-900 dark:text-gray-100 truncate flex-1">
										{domain}
										<span
											className={`ml-2 text-xs px-1.5 py-0.5 rounded-full ${status === "a" ? "bg-green-100 text-green-700 dark:bg-green-700 dark:text-green-100" : "bg-red-100 text-red-700 dark:bg-red-700 dark:text-red-100"}`}
										>
